...
button.button('reset')})});$('#pt_reset_password_form').on('submit',function(e){e.preventDefault();var button=$(this).find('button');button.button('loading');$.post(ptajax.ajaxurl,$('#pt_reset_password_form').serialize(),function(data){var obj=$.parseJSON(data);$('.pt-reset-password .pt-errors').html(obj.message);button.button('reset')})});if(window.location.hash=='#login'){pt_open_login_dialog('#pt-login')}});(function(jQuery,undefined){var push=Array.prototype.push,rcheck=/^(?:radio|checkbox)$/i,rplus=/\+/g,rselect=/^(?:option|select-one|select-multiple)$/i,rvalue=/^(?:button|color|date|datetime|datetime-local|email|hidden|month|number|password|range|reset|search|submit|tel|text|textarea|time|url|week)$/i;function getElements(elements){return elements.map(function(){return this.elements?jQuery.makeArray(this.elements):this}).filter(":input:not(:disabled)").get()}
...
$('.load_more_jobs',target).removeClass('loading');$('li.job_listing',results).css('visibility','visible')}
$(results).removeClass('loading');target.triggerHandler('updated_results',result)}catch(err){if(window.console){window.console.log(err)}}}},complete:function(){window.setTimeout(function(){var maxHeight=-1;var maxHeightb=-1;jQuery('li.job_listing').each(function(){maxHeight=maxHeight>jQuery(this).outerHeight()?maxHeight:jQuery(this).outerHeight()});jQuery('li.job_listing .position').each(function(){maxHeightb=maxHeightb>jQuery(this).outerHeight()?maxHeightb:jQuery(this).outerHeight()});jQuery('li.job_listing').each(function(){jQuery(this).height(maxHeight)});jQuery('li.job_listing .position').each(function(){jQuery(this).height(maxHeightb)})},200)},error:function(jqXHR,textStatus,error){if(window.console&&'abort'!==textStatus){window.console.log(textStatus+': '+error)}},statusCode:{404:function(){if(window.console){window.console.log('Error 404: Ajax Endpoint cannot be reached. Go to Settings > Permalinks and save to resolve.')}}}})});$('#search_keywords, #search_location, .job_types :input, #search_categories, .job-manager-filter').change(function(){var target=$(this).closest('div.job_listings');target.triggerHandler('update_results',[1,!1]);job_manager_store_state(target,1)}).on('keyup',function(e){if(e.which===13){$(this).trigger('change')}});$('.job_filters').on('click','.reset',function(){var target=$(this).closest('div.job_listings');var form=$(this).closest('form');form.find(':input[name="search_keywords"], :input[name="search_location"], .job-manager-filter').not(':input[type="hidden"]').val('').trigger('chosen:updated');form.find(':input[name^="search_categories"]').not(':input[type="hidden"]').val(0).trigger('chosen:updated');$(':input[name="filter_job_type[]"]',form).not(':input[type="hidden"]').attr('checked','checked');target.triggerHandler('reset');target.triggerHandler('update_results',[1,!1]);job_manager_store_state(target,1);return!1});$(document.body).on('click','.load_more_jobs',function(){var target=$(this).closest('div.job_listings');var page=parseInt(($(this).data('page')||1),10);var loading_previous=!1;$(this).addClass('loading');if($(this).is('.load_previous')){page=page-1;loading_previous=!0;if(page===1){$(this).remove()}else{$(this).data('page',page)}}else{page=page+1;$(this).data('page',page);job_manager_store_state(target,page)}
target.triggerHandler('update_results',[page,!0,loading_previous]);return!1});$('div.job_listings').on('click','.job-manager-pagination a',function(e){e.preventDefault();var target=$(this).closest('div.job_listings');var page=$(this).data('page');job_manager_store_state(target,page);target.triggerHandler('update_results',[page,!1]);$('body, html').animate({scrollTop:target.offset().top},600);return!1});if($.isFunction($.fn.chosen)){if(job_manager_ajax_filters.is_rtl===1){$('select[name^="search_categories"]').addClass('chosen-rtl')}
...
var location=document.location.href.split('#')[0];function job_manager_store_state(target,page){if($supports_html5_history){var form=target.find('.job_filters');var data=$(form).serialize();var index=$('div.job_listings').index(target);window.history.replaceState({id:'job_manager_state',page:page,data:data,index:index},'',location+'#s=1')}}
$(window).on('load',function(){$('.job_filters').each(function(){var target=$(this).closest('div.job_listings');var form=target.find('.job_filters');var inital_page=1;var index=$('div.job_listings').index(target);if(window.history.state&&window.location.hash){var state=window.history.state;if(state.id&&'job_manager_state'===state.id&&index===state.index){inital_page=state.page;form.deserialize(state.data);form.find(':input[name^="search_categories"]').not(':input[type="hidden"]').trigger('chosen:updated')}}
target.triggerHandler('update_results',[inital_page,!1])})})});document.addEventListener("click",function(){});function hmenu_activate_menu(menu_id,current_url){var add=jQuery(".add_to_cart_button");add.bind('click',function(){jQuery(document).on('ajaxSuccess',function(){get_product_count()})});hmenu_add_files(menu_id);get_product_count();set_active_state(current_url)}
