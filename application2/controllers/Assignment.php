<?php
   class Assignment extends CI_Controller {
    	...
    	public function index() {
            ...
            $data['alldata']=$this->Assignment_model->get_all_();
      ...
      if($this->session->userdata('username')  && $this->session->userdata('type') == 'ADMIN')
      {
            ...
            $data['alldata']=$this->Assignment_model->get_all_with_status('Pending');
            $data['allshopper']=$this->MysteryShopper_model->all_shopper();
            $data['user']= $this->session->userdata('username');
            $data['allclient']=$this->Client_model->get_all();
            $this->load->view("Header",$data);
...
//print_r($config);
      ...
      if($this->session->userdata('username'))
      {
            ...
            $data['alldata']=$this->Assignment_model->get_all_with_status('Reject');
            $data['allshopper']=$this->MysteryShopper_model->all_shopper();
            $data['user']= $this->session->userdata('username');
            $data['allclient']=$this->Client_model->get_all();
            $this->load->view("Header",$data);
      ...
      if($this->session->userdata('username')  && $this->session->userdata('type') == 'ADMIN')
      {
            ...
            $data['alldata']=$this->Assignment_model->get_all_with_status('Completed');
            $data['allshopper']=$this->MysteryShopper_model->all_shopper();
            $data['user']= $this->session->userdata('username');
            $data['allclient']=$this->Client_model->get_all();
            $this->load->view("Header",$data);
       ...
       public function add_assignment()
    {
     ...
     public function update_assignment()
     {
            if($this->session->userdata('username')  && $this->session->userdata('type') == 'ADMIN')
           {
            ...
            $id = urldecode($this->uri->segment(3));
            $data1['selected_assignment']=$this->Assignment_model->searchDetail($id);
